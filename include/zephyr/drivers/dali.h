 ...
 * @file
 *
 * @brief Public APIs for the DALI drivers.
 */
...
#define DALI_DT_SPEC_GET(node_id)         \
...
typedef int (*dali_api_configure_t)(const struct device *dev);
typedef int (*dali_api_get_config_t)(const struct device *dev);
typedef int (*dali_api_full_io_t)(const struct device *dev,
         struct dali_msg *msgs,
         uint8_t num_msgs,
         uint8_t addr);

__subsystem struct dali_driver_api {
  dali_api_configure_t configure;
  dali_api_full_io_t transfer;
};

#define DALI_DEVICE_DT_DEFINE(node_id, init_fn, pm, data, config, level, \
           prio, api, ...)        \
  DEVICE_DT_DEFINE(node_id, init_fn, pm, data, config, level, \
       prio, api, __VA_ARGS__)

#define DALI_DEVICE_DT_INST_DEFINE(inst, ...)    \
  DALI_DEVICE_DT_DEFINE(DT_DRV_INST(inst), __VA_ARGS__)
 ...
 */
static inline int dali_write(const struct device *dev,
          uint8_t addr,
          const uint8_t *buf,
          uint8_t num_bytes)
{
  ...
  const struct dali_driver_api *api =
    (const struct dali_driver_api *)dev->api;
 ...
 */
static inline int dali_write_dt(const struct dali_dt_spec *spec,
          uint8_t addr,
          const uint8_t *buf,
          uint8_t num_bytes)
{
  return dali_write(spec->bus, addr, buf, num_bytes);
}
 ...
 * @return a value from dali_write()
 */
static inline int dali_write_byte(const struct device *dev,
          uint8_t addr,
          uint8_t value)
{
  return dali_write(dev, addr, &value, 1);
}
 ...
 */
static inline int dali_write_byte_dt(const struct dali_dt_spec *spec,
          uint8_t addr,
          uint8_t value)
{
  return dali_write_byte(spec->bus, addr, value);
}
